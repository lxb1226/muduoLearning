...
AsyncLogging::AsyncLogging(const string& basename,
  ...
  : flushInterval_(flushInterval),
    ...
    basename_(basename),
    rollSize_(rollSize),
    thread_(boost::bind(&AsyncLogging::threadFunc, this), "Logging"), // thread绑定到threadFunc回调函数
    latch_(1),
    mutex_(),
    cond_(mutex_),
...
void AsyncLogging::append(const char* logline, int len)
{
  ...
  else  // 当前日志已满
  {
    ...
    else // 如果前端写入速度太快了，一下子把两块缓冲都用完了，那么只好分配一块新的buffer，作为当前缓冲区
    {
...
/*
如果buffers_为空，使用条件变量等待条件满足（即前端线程把一个已经满了的buffer放到了buffers_中或者超时）。
将当前缓冲区放到buffers_数组中。
...
void AsyncLogging::threadFunc()
{
  ...
  while (running_)
  {
...

    {
      ...
      /*
      无论cond是因何（一是超时，而是当前缓冲区写满了）而醒来，都要将currentBuffer_放到buffers_中。
      如果是因为时间到（3秒）而醒，那么currentBuffer_还没满，此时也要将之写入LogFile中。
      如果有一个前端buffer满了，那么在前端线程中就已经把一个前端buffer放到buffers_中了。
      此时，还是需要将currentBuffer_放到buffers_中（注意，前后放置是不同的buffer；
    ...
    */
    ...
    // 从buffersToWrite弹出一个作为newBuffer2
    if (!newBuffer2)
    {
      assert(!buffersToWrite.empty());
